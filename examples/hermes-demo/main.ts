...
// Path to openclaw.mjs. Can be overridden via OPENCLAW_MJS env var.
      ...
      (err, stdout) => {
        ...
        try {
          ...
          const reply =
            ...
            data?.text ??
            "";
          console.log(`[Gateway] ${elapsed}ms, ${reply.length} chars`);
...
async function main() {
  ...
  const agent: Agent = {
    async chat(text, participantId, signal) {
      ...
      console.log(`[AGENT] ${short} asking...`);
      const reply = await openclawChat(text, participantId, signal);
      console.log(`[AGENT] ${short} reply: "${reply.slice(0, 60)}${reply.length > 60 ? "..." : ""}"`);
      return reply;
    },
  };
  ...
  console.log("[READY] Hermes agent live in Discord.\n");
